...
class TestCompressionBaselines(unittest.TestCase):
    ...
    def test_high_fidelity_compactor_baseline(self) -> None:
        ...
        metrics = self._measure_fidelity(compress, decompress, self.test_embeddings)
        ...
        self.assertGreaterEqual(
            metrics["min"], 0.90, f"HighFidelityCompactor min fidelity {metrics['min']:.4f} < 0.90"
        )
    ...
    def test_dense_differential_store_baseline(self) -> None:
        ...
        self.assertGreater(
            mean_fidelity, 0.0, "DenseDifferentialMemoryStore should have positive fidelity"
        )
    ...
    def test_lossless_compactor_baseline(self) -> None:
        ...
        self.assertGreater(metrics["mean"], 0.0, "LosslessCompactor should have positive fidelity")
...
class TestCalibrationNeeds(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_embeddings = F.normalize(torch.randn(100, self.embed_dim), p=2, dim=-1)
    ...
    def test_calibration_assessment(self) -> None:
        ...
        for emb in self.test_embeddings:
            ...
            fid = F.cosine_similarity(emb.unsqueeze(0), recon.unsqueeze(0), dim=-1).item()
        ...
        self.assertGreaterEqual(
            hf_mean, 0.90, "HighFidelityCompactor should achieve at least 0.90 fidelity"
        )
