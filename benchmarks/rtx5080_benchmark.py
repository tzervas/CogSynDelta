...
def run_inference_benchmark(model: nn.Module, input_shape: Tuple[int, ...], device: torch.device, 
    ...
    for batch_size in batch_sizes:
        ...
        print(f"  Batch {batch_size:3d}: {avg_time_ms:7.2f}±{std_time_ms:5.2f} ms  "
...
def run_training_benchmark(model: nn.Module, input_shape: Tuple[int, ...], device: torch.device, 
    ...
    for step in range(num_steps):
        ...
        if (step + 1) % 50 == 0:
            ...
            print(f"  Step {step+1}/{num_steps}: {avg_time:.2f} ms/step  ({throughput:,.0f} samples/sec)")
...
def main() -> None:
    ...
    training_results = run_training_benchmark(
        training_model, (1, 28, 28), device,
        batch_size=128, num_steps=200
    )
    ...
    print(f"Best CNN Throughput: {best_cnn['throughput_samples_sec']:,.0f} samples/sec (batch {best_cnn['batch_size']})")
    
    best_resnet = max(resnet_results, key=lambda x: x['throughput_samples_sec'])
    print(f"Best ResNet Throughput: {best_resnet['throughput_samples_sec']:,.0f} samples/sec (batch {best_resnet['batch_size']})")
    
    print(f"\nTraining: {training_results['throughput_samples_sec']:,.0f} samples/sec")
    
    print(f"\n{'='*80}")
    print('✓ All benchmarks completed successfully!')
    print(f'Results saved to: rtx5080_benchmark_results.json')
    print('='*80)
