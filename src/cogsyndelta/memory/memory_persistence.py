...
class MemoryCompressor(nn.Module):
    ...
    def __init__(
        self, embed_dim: int = 512, compressed_dim: int = 256, compression_ratio: float = 0.5
    ) -> None:
    ...
    def cluster_and_merge(
        self, memories: torch.Tensor, similarity_threshold: float = 0.9
...
class PersistentMemoryBank(nn.Module):
    ...
    def write(self, content: torch.Tensor, importance: float = 1.0, source: str = "unknown") -> Any:
        """
        ...
        for i in range(batch_size):
            ...
            metadata = MemoryMetadata(
                timestamp=datetime.now(), importance=importance, hash=emb_hash, source=source
            )
        ...
        if self.use_dense_encoding:
            ...
            for i in range(min(self.working_pointer, self.working_capacity)):
                ...
                compression_stats = self.dense_store.compress_and_store(
                    embedding, memory_id=memory_id, importance=importance
                )
            ...
            merged, keep_indices = self.compressor.cluster_and_merge(
                self.working_memory[: self.working_capacity], similarity_threshold=0.85
            )
    ...
    def _archive_to_long_term(self) -> None:
        ...
        importances = torch.tensor([m.importance for m in self.short_term_metadata])
    ...
    def read(
        self, query: torch.Tensor, num_reads: int = 5, include_long_term: bool = False
    ) -> tuple[torch.Tensor, list[MemoryMetadata]]:
        """
        ...
        if working_valid.size(0) > 0:
            similarities = F.cosine_similarity(
                query.unsqueeze(1), working_valid.unsqueeze(0), dim=-1
            )
        ...
        if len(retrieved_list) < num_reads:
            short_term_valid = self.short_term_memory[
                : min(self.short_term_pointer, self.short_term_capacity)
            ]
            if short_term_valid.size(0) > 0:
                ...
                similarities = F.cosine_similarity(
                    query.unsqueeze(1), decompressed.unsqueeze(0), dim=-1
                )
    ...
    def save_checkpoint(self, checkpoint_path: str) -> None:
        ...
        checkpoint = {
            ...
            "pointers": {"working": self.working_pointer, "short_term": self.short_term_pointer},
...
if __name__ == "__main__":
    ...
    memory_bank = PersistentMemoryBank(
        embed_dim=512, working_capacity=10, short_term_capacity=50, storage_path="./test_memory"
    )
    ...
    print(
        f"  Working memory: {stats['working_memory']['used']}/{stats['working_memory']['capacity']}"
    )
    print(
        f"  Short-term memory: {stats['short_term_memory']['used']}/{stats['short_term_memory']['capacity']}"
    )
