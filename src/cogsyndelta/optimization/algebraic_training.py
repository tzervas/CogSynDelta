...
class UnifiedAlgebraicTrainer:
    ...
    def _optimize_mhc(self, train_x: Tensor, results: dict[str, Any]) -> None:
        """Optimize mHC modules if present.
    ...
    def _optimize_pathways(self, results: dict[str, Any]) -> None:
        """Optimize pathways if present.
    ...
    def _apply_predicted_weights(self, optimal_weights: dict[str, Tensor] | None) -> None:
        """Apply predicted weights to the model.

        Args:
            optimal_weights: Predicted optimal weights.
        """
        if optimal_weights:
    ...
    def train_algebraically(
        ...
        self._optimize_pathways(results)

        # Apply weights if requested
        if apply_weights and "optimal_weights" in results:
            self._apply_predicted_weights(results["optimal_weights"])
