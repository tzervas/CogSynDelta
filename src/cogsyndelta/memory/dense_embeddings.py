...
class DifferentialEncoder(nn.Module):
    ...
    def encode_differential(self, current: torch.Tensor, reference: torch.Tensor) -> torch.Tensor:
        """
    ...
    def decode_differential(
        self, differential: torch.Tensor, reference: torch.Tensor
    ) -> torch.Tensor:
        """
...
class AdaptiveQuantizer(nn.Module):
    ...
    def __init__(self, min_bits: int = 4, max_bits: int = 16) -> None:
        ...
        self.importance_estimator = nn.Sequential(
            nn.Linear(1, 32), nn.GELU(), nn.Linear(32, 1), nn.Sigmoid()
        )

    def quantize(
        self, values: torch.Tensor, importance: torch.Tensor | None = None
    ...
    def dequantize(self, quantized: torch.Tensor) -> torch.Tensor:
        ...
        dequantized = (
            scaled * (self.scale_info["max"] - self.scale_info["min"]) + self.scale_info["min"]
        )
...
class DenseDifferentialMemoryStore:
    ...
    def __init__(
        self, embed_dim: int = 512, dense_dim: int = 64, num_references: int = 100
    ) -> None:
    ...
    def compress_and_store(
        self, embedding: torch.Tensor, memory_id: str, importance: float = 1.0
    ) -> dict[str, Any]:
        """
        ...
        differential = self.diff_encoder.encode_differential(
            embedding.unsqueeze(0), reference.unsqueeze(0)
        ).squeeze(0)
        ...
        dense_quantized, bits_dense = self.quantizer.quantize(
            dense.unsqueeze(0), importance=torch.tensor([importance])
        )
        ...
        reconstructed = self.decompress_from_store_data(dense, residual, ref_idx)
        fidelity = F.cosine_similarity(
            embedding.unsqueeze(0), reconstructed.unsqueeze(0), dim=-1
        ).item()
    ...
    def decompress_from_store_data(
        self, dense: torch.Tensor, residual: torch.Tensor, ref_idx: int
    ) -> torch.Tensor:
        ...
        differential_decoded = self.dense_encoder.decode(
            dense.unsqueeze(0), residual.unsqueeze(0)
        ).squeeze(0)

        # Reconstruct from differential + reference
        reconstructed = self.diff_encoder.decode_differential(
            differential_decoded.unsqueeze(0), reference.unsqueeze(0)
        ).squeeze(0)
    ...
    def retrieve(self, memory_id: str) -> torch.Tensor:
        ...
        # Decompress
        reconstructed = self.decompress_from_store_data(dense, residual, data["reference_idx"])
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        avg_compression = np.mean([m.compression_ratio for m in self.metadata_store.values()])
        avg_fidelity = np.mean([m.fidelity_score for m in self.metadata_store.values()])
        ...
        compressed_size = sum(
            [
                (
                    ...
                    + data["residual"].numel() * data["bits_residual"]
                )
                / 8
...
if __name__ == "__main__":
    ...
    for i in range(20):
        stats = store.compress_and_store(
            test_embeddings[i], memory_id=f"mem_{i}", importance=0.5 + i * 0.025
        )
        if i == 0:
    ...
    fidelity = F.cosine_similarity(original.unsqueeze(0), retrieved.unsqueeze(0), dim=-1).item()
