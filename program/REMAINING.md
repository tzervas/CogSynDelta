...
`docs/design/AUDIO-CORPUS-AUDIT.md`). **Revision 3.5 (2026-09-03) is the current text** — see the *revision 3.5* block below for what it adds. Revision 3.4 is the text it extends, and it
...
**REVISION 3.5 (2026-09-03) — what it adds to this file.** Ten new decisions, **DEC-68 to DEC-77**,
and **six new rows mirrored id-for-id below**: **W4n** (the negatives ablation, blocked on OD-17),
**W7k** (the knob table as receipt fields, plus the runtime packing prototype), **E1a** (the VRAM
arbiter and the frugality cap), **P2′g** (per-faculty token targets, clean-terms generators,
enrichment), **P5′d** (differential activations) and **P5′p** (predictive hybrid training).
**Two existing rows have their STATUS cells corrected by measurement and their gates untouched:**

- **W4 HAS RUN AT ITS PRE-REGISTERED CONFIGURATION.** Batch 1280, `token_loss_chunk` 512, 4,000
  steps, receipt `/akula-data/session-backup-staging/w4-chunked/run-1280/memory-20260903T184441Z.json`,
  code `eb735ab`. **(a) PASS** (`recall@1` 0.8535 vs parents 0.7754/0.7559; graded 0.7893 vs 0.7588),
  **(b) PASS AS SCORED, AND CONTINGENT** (full-pool `recall@10` 0.200 at a **strict** `> 0.20`
  floor; MRR 0.1168 at a 0.10 floor — the receipt's value is `0.20000000298023224` =
  `float(numpy.float32(0.2))`, so the float32 recall *is* 0.2 and the whole margin is
  representation error; a strict reading makes this an exact tie and the run **two passes and
  three fails**. **OD-17 answers `>` versus `≥`**, not this file),
  **(c) FAIL** (0.200 vs BM25 0.440), **(d) PASS**, **(e) FAIL** (rank ratio 1.2102 < 2.0; the
  regression clause passes at 0.0). **The negatives curve, batch 256/512/1280:** full-pool
  `recall@10` **0.030/0.098/0.200**, held-out `recall@1` **0.600/0.815/0.854**. **The row's five
  gate clauses are NOT edited** — the pivot-or-amend decision is **OD-17**, operator to confirm, and
  **W4n is blocked on it**.
- **W7p's tooling EXISTS and lives in `tzervas/gpu-pack`** (`main` at `5364932`, rounds 1–4 merged
  through PRs #1 and #2) under the tooling-lives-in-its-own-repo rule, **DEC-75**. CSD's side is the
  thin adapter already in this tree. **P10.4's "the probe/admit/launch pipeline is the remaining
  piece" is retired by that merge** and its status is corrected below.

**Also recorded, without new rows:** **DEC-74** — all three cards and all their runtimes are **on
demand**; an idle `:8080`, a stopped unit or an exited container is the normal resting state, and
the **1080 Ti cannot run the pinned `torch 2.11.0+cu128` at all** (`get_arch_list()` floors at
sm_75; driver 535.274 caps at CUDA 12.2), while a separate `torch==2.5.1+cu121` venv there works —
so its default role is inference/RAG/utility. **DEC-76** — the autodev spec pointer moves to **rev 5
at `ece346b`** (PR #1 in `tzervas/csd-autodev`), and the rule that came out of five rounds is
*freeze a code revision, then write the next spec revision against it, then implement*. **DEC-77** —
PR-only extends to `gpu-pack`; the CI secret scan has a **prose false-positive class** whose remedy
is a path-and-phrase-scoped allowlist verified by planting a real-shaped secret; and **the CI runner
has no GPU tooling**, so tests must not assume `nvidia-smi` and a diagnostic subprocess must degrade
one field rather than abort its caller. **OD-18** collects the dataset factory's five legal readings;
its first item needs no lawyer — the model card must state the CC BY-NC-SA reading it takes.

The old router-shaped P4/P5/P6 rows are replaced below by
...
| **W7v** | **`visual`: token-aware retrain AND resolution rebuild** `[A4 fixed]`. `visual` **physically cannot ingest a composite image today**: `JEPAConfig(image_size=64, patch_size=8)` and `ViTEncoder` registers a **fixed** sincos `pos_embed` of size `cfg.n_patches = 64` (`model/vl_jepa.py:52-53, 182, 199` [V]), so `h = self.patch_embed(x) + self.pos_embed` is a **shape error** on a larger image, not a slower forward. Meanwhile §5.4's `visual × language_code` and `visual × memory` are **2 of 4 cross-faculty pairs = 1,024 of 2,048 eval items**, and §5.4 itself concedes they are *"unconstructible against 64×64 single-object tiles"*. Revision 1 scheduled no row to build the prerequisite. **This row does it, folded into the retrain that W1's result made mandatory anyway, so the marginal cost is the resolution change and not a separate run.** Re-shape to composite resolution (`image_size 128`, `patch_size 8` ⇒ `n_patches 256`, `pos_embed` regenerated or 2-D interpolated), continue-train, and — if OD-4 is decided that way — swap the corpus in the same run. **`pos_embed` REGENERATED, not 2-D interpolated — the study behind this row killed "or 2-D interpolated" as a category error** (a length-64 1-D raster has no `(x,y)` axes to bicubic; see below). | New `n_patches`, `kv_bytes_per_token` and `token_budget.max` recorded and propagated to §1.4, §2.3 and §6.1. Probe top1 and cifar100 transfer **do not regress by more than 1 point** against 0.0606 / 0.2625 **measured on the target encoder** (DEC-34), with the untrained baseline re-instantiated at a region-specific seed. **§4.0's retrain gate**, both clauses. **A composite frame renders, encodes and round-trips** — the negative test is that the 64×64 checkpoint **raises** on the same input. **The frame is W3r's checked-in 128×128 fixture, NOT a W3 deliverable** `[N5 fixed]`: revision 2's gate consumed the composite renderer, which is W3's primary deliverable, while W3 was `blocked_by: W7v` — a dependency inversion in which neither row could start. W3r breaks it. **W7v-cfg landed the CONFIG-LEVEL HALF of this row 2026-09-04, CPU only, no GPU training** `[OP: g8-visual-faculty-design.md "What to build first"; g8-visual/S02.md]`: `JEPAConfig` imports `FRAME_SIZE`/`PATCH_SIZE`/`N_PATCHES` from `vl/composite.py` rather than re-deriving them, so the default config now renders the 16×16/256-patch grid this row calls for; positions are a genuine **2-D sincos** (`sincos2d_pos_embed`) recomputed for `cfg.grid` at construction — the 1-D table (`sincos_pos_embed`) is untouched and still correct for `TextEncoder`, which is its only remaining caller; an explicit 64-px config still works (steal vs. interpolate, per S02 §5.2). Loading a checkpoint recorded at one grid into a config built for another now **raises**, naming both grids (`check_checkpoint_grid_compatible`) — closes the silent-load hole `pos_embed` being a non-persistent buffer otherwise leaves open. `VL_REGIONS["vl_latent"]` no longer hardcodes tiny-imagenet: `corpus_source` must name a landed, admitted corpus or `run_vl_region` refuses to start, citing OD-4. The visual receipt now stamps `corpus.fingerprint`/`corpus.fingerprint_scheme` (reusing `cogsyndelta.corpus.fingerprint_corpus`, no new scheme) and prints `masking: random-permutation` (the truth about `sample_masks`; multi-block masking is NOT implemented). **Still pending, and this row is not done:** OD-4's corpus is not landed, so no retrain has run under the new geometry; the 0.0606/0.2625 numbers above are still the pre-rebuild 64-px/tiny-imagenet receipt and are void as a gate the moment OD-4 replaces the corpus (re-instantiate untrained + chance on the new probe, per the row's own gate). | in progress — config rebuild done 2026-09-04 (branch `feat/w7v-cfg`); retrain BLOCKED on OD-4 (corpus not yet landed) | 3090 `.98` |
...
### P2′f follow-through — the 2026-09-03 dataset-factory survey's open rows

The survey pass ran: eight faculty surveys, eight adversarial verification passes, and the
enrichment/licence analysis. Output is committed at
`docs/design/DATASET-FACTORY-CATALOGUE-2026-09-03.md` (**159 candidates**, 105 VERIFIED,
26 CONTRADICTED, 18 UNVERIFIABLE, 10 REFUSED-CLOSED; 106 provenance groups),
`docs/design/datasets/catalogue-2026-09-03.json` (machine-readable, ground shape),
the new `LICENCE-FOR-OPEN-WEIGHTS.md` §"Enriched and derived datasets", and the full evidence at
`docs/design/evidence/dataset-factory-2026-09-03/`. **These three rows are what the pass left
open.** They are additions under P2′f, not a renumbering of it or of anything else.

The headline measurement, because it decides what the next fetches are for: against the
**1e10-tokens-per-region** target, and computing the largest **B1-compliant** total each tier can
supply (`T_B1 = min(L+R, R/0.60)` for a largest provenance group `L` and the rest `R`) —
`language_trunk` reaches **6,359%**, `numeric_math` and `reasoning` reach **≥100% but only by
running the DeepMind `mathematics_dataset` generator** (2.6% without it), `language_code` reaches
**15%**, `memory` **5.0%** (2.0% if MS MARCO is refused), `moral_safety` **2.4%** (which is not a
shortfall — DEC-59 curates), and **`visual` reaches 0.3%**. And **NC-inclusive buys almost
nothing over clean-permissive**: 0 points for `language_code`, `visual` and `moral_safety`, 0.8
for `reasoning`, 3.7 for `memory` of which 3.3 is MS MARCO alone. **B1, not licence tolerance, is
the binding constraint** — the lever is more independent permissive provenance groups, not more
NC volume.

| id | task | gate | status | host |
|----|------|------|--------|------|
| **P2′f-a** | **THE NEXT TEN FETCHES, in the catalogue's priority order** `[DEC-57]`. Ranked by `T_B1` unlocked per unit of effort, not by dataset size. (1) **The OpenCoder technical report's data-availability section** — the only large code source with provenance independent of PG-STACK/PG-CSN/PG-COMPETITIVE (147.9 GB, 1.0e8 rows); it is what makes `language_code`'s B1 satisfiable at all. (2) **The Wikimedia Commons / WIT per-file licence sampling audit** (Commons API `imageinfo` + `extmetadata`) — the only non-dead-end in `visual`. (3) **Run the DeepMind `mathematics_dataset` generator at controlled category caps** — an action, not a fetch, and the single source in the catalogue that supplies arbitrary B1-relief volume under a settled Apache-2.0 grant. (4) **`bigcode/commitpackft`'s per-row `license` distribution** — the card claims blanket "permissive" while its own enum contains `agpl-3.0` and `unknown`. (5) **The `nickrosh/Evol-Teacher` conflict** (HF `cc-by-nc-sa-4.0` vs GitHub `Apache-2.0`, both VERIFIED) — unblocks two entries. (6) **`allenai/qasper`'s second independent primary** — `allenai.org/data/qasper` 302s to the HF card, so R1 fires. (7) **FigureQA's MSR data-download terms and PlotQA's MIT-vs-CC-BY-4.0 discrepancy** — ~404k synthetic charts, the only chart sources clean of the per-photographer trap. (8) **Resolve Natural Questions' grant scope** — see P2′f-b's finding. (9) **`courtlistener.com/terms/` from a different egress** — re-opens or closes 10M+ opinions. (10) **One successful SemEval-STS primary read** — clears five candidates at once and fills `compress`'s graded gate. | **Per fetch, and the balance rules are part of the gate, not a note beside it.** (i) The entry's `verification_status` moves off `UNVERIFIABLE` **with a verbatim quote, a URL whose host differs from the mirror's, and a fetch date** — or it stays `UNVERIFIABLE` and stays inadmissible; a fetch that fails is recorded as a failure, not as an inference. (ii) **Every admission names its provenance group and states its share against the B1 cap of 0.40 and `N_eff` against the B2 floor of 3**, computed post-dedup on groups; a share printed without its cap is refused. Specifically: fetch 1 must be capped ≤ 0.40 and deduped against PG-STACK first; fetch 2 must record the licence **per file** and quarantine CC BY-SA into a separate tier file; fetch 3 counts as **ONE** group however many categories are generated, with category as the B5 stratum key and a recorded sample method + seed (B4, never a prefix); fetch 4's filter must be **per row**, with the surviving `license` value written into the emitted row's `lic_class` column, not a sidecar; fetch 7's two candidates join the **existing** `synthetic-chart-render` group with CLEVR/Shapes3D and must not be counted as independent of it; fetch 8's answer does not change NQ's group — it joins `wikipedia` alongside SQuAD/HotpotQA/FEVER/DBpedia/MIRACL/Mr.TyDi either way. (iii) **No emitted file mixes tiers** — the SA×NC cell is empty and there is no licence that satisfies both, so a merge is refused (R5), not resolved. | **todo — CPU, no GPU** | 1080 Ti / CPU |
| **P2′f-b** | **THE LICENCE READINGS THE OPERATOR SHOULD OBTAIN** `[DEC-57] [DEC-31]`. Eight questions, each stated precisely enough to be billable, in `DATASET-FACTORY-CATALOGUE-2026-09-03.md` §10.1. Ranked: (1) **Is CC BY-NC-SA 4.0 coherent for the composed model given its CC BY-SA inputs?** — the only question that could invalidate a **shipped** artefact rather than constrain a future one. The ratified licence is coherent only under the permissive reading of the weights question; under the cautious reading SNLI's §3(b)(1)/§3(b)(3) and GooAQ's NC term have **no common compliant release licence at all**. The project has not stated which reading it takes, and this document already says such a position must be taken explicitly in the model card rather than by silence. (2) **Do OpenAI's and Anthropic's output restrictions bind a third party who receives the dataset?** — one decision covering **eleven** catalogue entries carrying a `MODEL-OUTPUT-TERMS` flag (KodCode, CodeFeedback, Evol-Instruct-Code, SynCSE-scratch-NLI, camel-ai/math, MetaMathQA, orca-math, NuminaMath's orca_math subset, Alpaca, UltraChat, ToxiGen), worth ~1e9 tokens across three faculties. (3) **MS MARCO specifically** — "we may not own the underlying rights in the documents" is closer to the operator's REFUSE class than to plain NC, and it is 3.3 of `memory`'s 5.0 NC-inclusive percentage points. (4) **Does Stack Exchange's per-item attribution clause reach an inference-time weight release?** — if not, the largest single lever against GooAQ's 79.1% concentration becomes available; if so, R9 refuses it structurally. (5) **Is a model a "Produced Work" under ODbL/ODC-By?** — bears on FineWeb-Edu, C4, peS2o, OpenWebMath, WildGuard and SciFact's corpus half. (6) **ToxiGen's CDLA-Permissive-2.0 grant vs its research-only README.** (7) **Do sui generis database rights apply to a US-domiciled operator?** (8) **How far does Gemma's "Model Derivative" definition reach through synthetic data?** Plus three corrections this pass owes to documents it does not own: `00-ground.md`'s `memory` row lists FiQA as CC-BY-SA-4.0 when the primary says **NC**; `10-survey-compress.md` contradicts itself on `ltg/en-wiki-paraphrased`; and `maveriq/bigbenchhard`'s card asserts the upstream BIG-bench repo is MIT when it is Apache-2.0. | **Three, and the first is a writing task, not a legal one.** (i) **The model card states the reading it takes on the weights question, explicitly**, with the consequence named — a silence here is the defect this document already flagged, and closing it does not require a lawyer. (ii) **Each of the eight questions is either answered and its answer recorded beside the affected catalogue entries, or it is recorded as still-open with the entries left inadmissible** — an entry may not quietly become admissible because the question got tiring. (iii) **The three corrections land in the documents that carry the wrong claim**, not only here; and no downstream row cites `maveriq/bigbenchhard`'s MIT claim or `00-ground.md`'s FiQA line as if it had been checked. | **todo — operator/legal, no GPU** | n/a |
| **P2′f-c** | **THE MORAL-CORPUS PROBE DESIGN — measure the behaviour change, never assume it** `[DEC-59]` (the measurement half of **P5′m**, specified here because the catalogue now names the corpus and the probes). **Two arms, one difference:** train the `moral_safety` **clean-permissive tier alone** (`T_B1` 2.4e8 tokens over 7 provenance groups — ETHICS, HH-RLHF, Moral Stories, Scruples, HateXplain, Civil Comments, Jigsaw), then the **NC-inclusive union** (adding the PKU NC file and the Social Chemistry share-alike file **as separate files**, per R5). The difference between the arms is the measured price of the NC and SA inputs — `measure-the-thing-not-the-proxy` applied to a licence decision, and the reason a separable permissive tier per faculty exists at all. **Probe set, pre-registered and held out:** MoralChoice (purpose-built to elicit revealed moral preferences under low- and high-ambiguity dilemmas), ETHICS' held-out slice, Measuring Hate Speech (its IRT-aggregated **continuous** severity score is the best calibration instrument in the catalogue), Civil Comments, OpenAI Moderation Eval, StereoSet, CrowS-Pairs, and Jigsaw Toxicity as a cross-taxonomy check against Civil Comments. **Two entries must be settled before the corpus is built, not after:** ToxiGen's CDLA-vs-README conflict, and whether `ucberkeley-dlab/measuring-hate-speech`'s CC-BY-4.0 grant covers only D-Lab's annotations or also the underlying Twitter/Reddit/YouTube post text — the second matters much less if it is used only as a probe, which is what the catalogue recommends. | **Four, and the first two are what make the result mean anything.** (i) **The untrained baseline is measured and reported for every probe**, at a region-specific seed — a delta without one is not interpretable, and W2c already retired one disputed floor for exactly this reason. (ii) **B3 is declared as `held-out-domain` and checked mechanically**: every probe's provenance group contributes **zero** training rows, verified against `train_source_shares` vs `eval_source_shares` rather than against a comment, **and an in-mixture number is reported beside it**. (iii) **The probes are pre-registered before either arm runs** — a probe chosen after seeing the arms is a proxy, not a measurement. (iv) **The two arms are reported together with their corpus sizes and B1/B2 numbers beside their thresholds**, so "the NC input was worth N points" is a measured statement about a capability delta and not about a share. **The corpus does not target 1e10 tokens** — DEC-59's requirement is *"extremely high quality and requirement-complete over large"*, and the catalogue's 2.4% figure is recorded for comparability, not as a shortfall to close. | **todo — GPU for the two arms, CPU for the corpus build; blocked_by P2′f-b's ToxiGen and Measuring-Hate-Speech answers for the training arms only, not for the probe-only use** | 5080 / 3090 Ti |
| **P2′f-d** | **THE CogSynDelta-SIDE INGEST ADAPTER for the `tzervas/dataset-factory` tool** (worktree `csd-worktress/CogSynDelta-wt-dataset-ingest`, branch `feat/dataset-ingest-adapter`). `program/datasets/README.md` documents the seam: how the factory consumes `docs/design/datasets/catalogue-2026-09-03.json`, where a fetch lands (`/bulk/csd-corpus/factory-<date>/<faculty>/<dataset-id>/`, never overwriting an existing region path), and the three-check admission gate a fetched dataset must clear before joining a region's training config — licence tier vs. the region's `LICENCE_TIER` (`scripts/csd-publish-checkpoint.py`), post-admission provenance-group share against B1's 0.40 cap, and `verification_status == VERIFIED`. `scripts/csd-corpus-admit.py` implements the gate as a read-only, non-mutating checker (prints PASS/FAIL per check, exits nonzero on any FAIL) — it does not fetch, and it does not touch `CORPUS-CONTRACT.md`, `csd-regions.json`, or a region's fetch list. **This pass fetched nothing** — the next fetch batch is still P2′f-a's ten-item list, now with a landing pad and an admission gate to write through. | **Two.** (i) **The three checks can each be made to fail, not only to pass** — `tests/test_csd_corpus_admit.py` constructs one failing case per check (a stricter-than-region verdict, an over-cap provenance-group share including the boundary case, each non-VERIFIED status) alongside the passing cases, per `verify-guards-by-making-them-fail`. (ii) **The transcribed `REGION_TIER` table cannot silently drift from `scripts/csd-publish-checkpoint.py`'s `LICENCE_TIER`** — one test loads both modules and asserts equality, so a future edit to either table without the other fails CI rather than being admitted on a stale tier. | **done — CPU, no GPU; targeted tests/ruff/mypy/ci_local.sh green, not pushed** | CPU |
...
| P7.3 | **Train -> test -> quantize -> test matrix**, config-driven (`program/matrix/csd-matrix.yaml`), every variant published to `tzervas/cogsyndelta-region-<region>` with a versioned identifier; see `docs/design/MATRIX-PIPELINE.md`. CSD's side (this row): `csd-benchmark.py --quantized` scores the packed artifact itself (`kind: eval-quantized`, bound to `quantized_sha256` **and** `checkpoint_sha256`), every stage receipt carries `kind` + `started_utc`, `corpus_fingerprint` is a top-level receipt field. The harness (wave scheduling, HF branch/tag publish, the comparison table) is `tzervas/model-matrix`, a separate repo per `tooling-lives-in-its-own-repo` — tracked there, not here. | `--quantized` reproduces `csd-quantize.py`'s own `quantized_metric` for today's memory V2 artifact within 1e-4 (`tests/test_benchmark_quantized_artifact.py`); fp32 and quantized receipts differ in `kind` and in `artifacts` binding; `program/matrix/csd-matrix.yaml` validates and names all 23 first-matrix cells (code/compress/retrieve/reason/memory) | wip — CSD-side adapter done (`feat/matrix-config`); harness repo and the actual matrix run are separate, unstarted work |
| P7.4 | **`csd-metrics/v2` harness consumer round**: `docs/design/METRICS-METHODOLOGY.md` §§12–19 (2026-09-04) define the canonical field names, the schema stamp + refuse predicate, the retire list, and the v1→v2 deprecation map, but no code writes `metrics_schema` or applies the refuse predicate yet — that patch is this row. Also: the encoder adapter (live `EncoderProtocol.encode(DataLoader, ...)`, FiQA nDCG@10 beside W4 recall/MRR in `detail.external`, never aliased into `rank.*`/`beir.*` collision) belongs in a **separate `csd-eval-bridge` repo** (candidate: extend `tzervas/model-matrix`, which already reads `metrics["rank.recall@1"]` and must not import a project package per its own `DESIGN.md` §1) — CogSynDelta keeps `TextEncoder.encode` and its receipts; the bridge depends on CSD as a library, per the operator's `tooling-lives-in-its-own-repo` rule (same shape as P7.3's matrix harness). | a `compare()`-shaped refuse function in `cogsyndelta.eval.metrics` rejects two receipts that differ on any of schema/fingerprint/battery_id/k/pooling/sha/region/git_sha/seed, and accepts the one licensed cross-`battery_id` special case (`quant.plan_recall@1` vs. `quant.artifact_recall@1`, same sha/holdout) — MM §14's three schema falsifiers all fail to reproduce; every training/eval/quant receipt stamps `metrics_schema: "csd-metrics/v2"`; the bridge repo exists, runs FiQA through MTEB **and** `beir_fiqa.py` on the same qrels, and writes `detail.external` in the shape MM §18 specifies | todo — design-only per MM §18 (no mteb/beir/datasets/sklearn/lm-eval/helm dependency in `pyproject.toml` today, confirmed by grep); needs the refuse-predicate implementation in this repo before the bridge repo's adapter has anything to cross-reference against |
| P7.5 | **Rename the live Hub repo, then delete the `hub_repo_name` pin.** DEC-78's rename (`code` → `language`) is deliberately NOT reflected on the Hub yet: `default_repo("language")` already returns the future name `tzervas/cogsyndelta-region-language` (a repo that does not exist), while `default_repo("code")` keeps returning the live one, `tzervas/cogsyndelta-region-code` — both pinned by `tests/test_publish_checkpoint.py:366-372` and documented at `scripts/csd-publish-checkpoint.py:164-172,295-305`. This is safe today only because `program/matrix/csd-matrix.yaml`'s `hub_repo_name` override pins the matrix harness at the legacy name (`tests/test_matrix_config.py:681`) — an operator who follows the renamed docs and runs `--region language` **without** `--repo` gets a freshly `create_repo(exist_ok=True)`-provisioned empty repo, not a publish to the live one. This row is: (1) rename the Hub repo `cogsyndelta-region-code` → `cogsyndelta-region-language` with a redirect (HF Hub supports repo moves), (2) drop the `hub_repo_name` override from `csd-matrix.yaml` once `default_repo("language")` and the live repo agree, (3) update the two test files above to assert the post-rename identity instead of the pre-rename split. | `default_repo("language")` and `default_repo("code")` both resolve to the same live, redirect-backed repo; `hub_repo_name` no longer appears in `csd-matrix.yaml`; `tests/test_matrix_config.py` and `tests/test_publish_checkpoint.py` assert the merged identity | todo — orchestrator step, separate from CSD-side code; flagged by the `refactor/language-centre` review (2026-09-04) as documented only in source comments, tracked nowhere before this row |
