...
`cogsyndelta.cards.render_card` (`src/cogsyndelta/cards/render.py:376`) is the newer,
...
`render_card` (`src/cogsyndelta/cards/render.py:376`) is the one public entry point.
...
|---|---|---|
| undocumented metric | `CardError` (`require_documented`, `src/cogsyndelta/cards/methodology.py:762`) | a receipt carries a metric key with no entry in `METRIC_METHODOLOGY` — no stated formula/battery/source to print beside it |
...
| unresolved licence tier | `CardError` (`_licence_block`, `src/cogsyndelta/cards/render.py:346`) | any `kind` except `placeholder`/`composed` with no `region_cfg["licence_tier"]` set — a `placeholder` (no weights) or `composed` (licence resolution is the operator's call, CARD SPEC §9) renders a stated placeholder line instead |
