...
| Chroma vector store | `storage/vector_store.py` exists and now binds model id/dimension metadata. | No Chroma equivalent required. | **KEEP, HARDEN**: retain only if it passes the common contract and restart tests. |
