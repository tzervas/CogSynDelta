...
| P7.5 | **Rename the live Hub repo, then delete the `hub_repo_name` pin.** DEC-78's rename (`code` → `language`) is now reflected on the Hub: the orchestrator renamed `tzervas/cogsyndelta-region-code` → `tzervas/cogsyndelta-region-language` via `HfApi.move_repo` (private, old id redirects). `default_repo()` in `scripts/csd-publish-checkpoint.py` canonicalizes its input before the repo-name lookup, so `default_repo("code")` and `default_repo("language")` agree; `program/matrix/csd-matrix.yaml`'s `hub_repo_name` override is gone now that the row's own `repo_pattern` substitution resolves to the live repo. | `default_repo("language")` and `default_repo("code")` both resolve to the same live, redirect-backed repo; `hub_repo_name` no longer appears in `csd-matrix.yaml`; `tests/test_matrix_config.py` and `tests/test_publish_checkpoint.py` assert the merged identity | done (2026-09-05) — Hub rename evidence (before/after ref listings, redirect check) at `/akula-data/session-backup-staging/phase-d/hub-rename/`; landed in `refactor(publish): default_repo canonicalizes the region; both spellings publish to cogsyndelta-region-language` and `chore(matrix): drop the language row's hub_repo_name pin now that the Hub repo is renamed`; `tzervas/model-matrix` at `7008ea9` still resolves the region via its first alias (`cogsyndelta-region-code`, which now redirects) until its own fix for full `owner/name` `hub_repo_name` values lands |
