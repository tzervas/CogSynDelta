...
def benchmark_throughput(model, input_shape, device, batch_sizes=[1, 8, 32, 128], num_iterations=100):
    ...
    for batch_size in batch_sizes:
        ...
        print(f"  Batch {batch_size:3d}: {avg_time:7.2f}±{std_time:5.2f} ms  "
...
def benchmark_memory_bandwidth(device):
    ...
    for size_mb in sizes_mb:
        ...
        print(f"  {size_mb:4d} MB: H2D: {h2d_bandwidth:6.2f} GB/s, "
              f"D2H: {d2h_bandwidth:6.2f} GB/s")
...
def main():
    ...
    try:
        ...
        all_results['mnist_training'] = f"Failed: {str(e)}"
    
    # 2. CNN Throughput
    try:
        cnn_model = MNISTNet().to(device)
        cnn_results = benchmark_throughput(cnn_model, (1, 28, 28), device, 
                                          batch_sizes=[1, 16, 64, 256])
        all_results['cnn_throughput'] = cnn_results
    except Exception as e:
        print(f"CNN throughput benchmark failed: {e}")
        all_results['cnn_throughput'] = f"Failed: {str(e)}"
    ...
    try:
        ...
        all_results['resnet_throughput'] = f"Failed: {str(e)}"
    ...
    if device.type == 'cuda':
        try:
            ...
            all_results['memory_bandwidth'] = f"Failed: {str(e)}"
    ...
    if 'cnn_throughput' in all_results and isinstance(all_results['cnn_throughput'], list):
        ...
        print(f"\nCNN Inference:")
        print(f"  Best Throughput: {best_cnn['throughput']:.0f} samples/sec (batch {best_cnn['batch_size']})")
    ...
    if 'resnet_throughput' in all_results and isinstance(all_results['resnet_throughput'], list):
        ...
        print(f"\nResNet-18 Inference:")
        print(f"  Best Throughput: {best_resnet['throughput']:.0f} samples/sec (batch {best_resnet['batch_size']})")
