    ...
    def _check_docstrings(self, tree: ast.AST, filepath: str):
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                self.stats['total_functions' if isinstance(node, ast.FunctionDef) else 'total_classes'] += 1
...
class IntentionValidator:
    ...
    def _validate_function_intention(self, node: ast.FunctionDef, 
...
if __name__ == '__main__':
    ...
    exit(0 if result['status'] in ['ready', 'acceptable'] else 1)
