...
**What revision 3.6 additionally folds (g11 pack, 2026-09-05).** The g11 PARENT numbered nine new
rows DEC-78…DEC-86 against a rev-3.5 tree that had not yet taken the faculty-naming rule. **This
document already used DEC-78 for that rule** (landed with the language-centre rename at
`7867606`). g11's **DEC-82 (naming + minimal alias) is therefore folded INTO DEC-78 as an
amendment**, not minted as a second DEC-78; the rest of that PARENT is appended in numeric
order as **DEC-79 to DEC-86**, with g11's DEC-78…81 shifted up one. Amendments sit beside the
original text the same way DEC-56/DEC-63 do. **No GPU row is licensed by this fold** except
restating W7v's already-blocked retrain.

**(a) DEC-78 AMENDED: EARLY-ALPHA COMPAT IS THE MINIMAL ALIAS TABLE ONLY.** Canonical id
`language` (spoken "language centre" is **not** a third `name:` field); domains are never
region names; the runner may accept `code` / `vl_latent` and immediately share the same dict
entry; **refuse** dual live keys in the matrix; **no** forever `was:` JSON. The mapping in this
tree is `REGION_ALIASES = {"code": "language", "vl_latent": "visual"}`
(`src/cogsyndelta/regions/aliases.py:40-43` `[V]`).

**(b) AFFECT ISOLATION (DEC-79).** Amends the §1.3 `[B,8]` gain cell: tagged `z_affect`, reason
weights not conditioned, loud disconnect is a zero-gate. DEC-06's GoEmotions probe **stays a
probe**. G0–G5 are unrun. Default: not a workspace participant (OD-20).

**(c) VALUES AS BASE-TRAINING (DEC-80).** Amends DEC-59's *location*, not its probes or its
not-toy train clock: the default-mind moral basin lives in **base** weights so overlay drop
cannot erase it. Declare now; train after H1 (OD-22).

**(d) OVERLAY COMPOSE/RESTORE (DEC-81).** Amends DEC-33 / P5′o's receipt shape: compose `W+ΔW`
at matmul time; unload restores base bit-exactly on the SWITCH receipt. Do not write SWITCH
files into this tree unless a later test PR. Do not pull full P5′o into E1 (DEC-66).

**(e) τ / STREAMING SEAM (DEC-82).** Multi-axis temporal grounding is a declared discovery seam
paired with streaming latent mode as a **scheduler policy** under DEC-47. Not a region, not a
§4.1 GPU row (OD-25). `blocked_by`: writable store (E1) + H1.

**(f) W7v-cfg LANDED (DEC-83).** Config half is on `main` at `8fbd25b` (PR #36): defaults
128/8/256, 2-D sincos recompute, `corpus_source is None` refuse. Visual **retrain remains
blocked on OD-4**. Do not warm-start `step-8000.pt`. The W7v cell's "2 of 4 = 1,024 of 2,048"
sentence is a leftover against §5.4's live six-bin / 3,072 / ten-pair arithmetic.

**(g) `csd-metrics/v2` SHIPPED (DEC-84).** `METRICS_SCHEMA_V2 = "csd-metrics/v2"`
(`src/cogsyndelta/regions/_receipt.py:57,206` `[V]`); `compare()` is the refuse-function
(`src/cogsyndelta/eval/metrics.py:765-845` `[V]`). Envelope stays
`model-pipeline-receipt/v1`. Merged `86532f2` (PR #33); g8's "compare() has no identity
predicate" is retracted as present-tense.

**(h) PROCESS ROLES BIND; ADMIT ≠ TRAIN (DEC-85).** Every factory emit is `train` / `probe` /
`refuse`. Admission 2026-09-04 8/8 ADMIT does **not** glob a train mix. Faculty→region wiring
for the admit adapter is OD-24.

**(i) HARNESS REPOS STAY SPLIT (DEC-86).** CogSynDelta keeps the model; `tzervas/gpu-pack`,
`tzervas/csd-autodev`, `tzervas/agent-workflows` keep their roles. Do not donate CSD. Do not
mint a second workflow repo. Early-alpha publishing uses DEC-78's same minimal alias (OD-19).

**Open operator list gains OD-19 to OD-25.** OD-4, OD-16, OD-17, OD-18 remain open. **§5.4–§5.6
ablation-pair identifiers and per-faculty eval bins** re-key `language_code` → `language`;
dated measurement citations that name the old spelling stay as history.
...
| **DEC-06** | `classify_go_emotions` → probe of a new declared `affect` faculty. **KEPT IN REVISION 3.6 BY DEC-79:** the probe stays a probe; isolation is a tagged stream, never a promotion of this head | §1 |
...
| **DEC-33** | **Memory-gate overlays** are the dynamic-paging seam's first client: tiered residency, fingerprint-bound attach, loud disconnect, strictest-input licence. A first-class manifest artifact type. **AMENDED IN REVISION 3.6 BY DEC-81:** the loud-disconnect receipt is SWITCH-shaped — compose `W+ΔW` at matmul time, never `param.data +=`; unload restores base bit-exactly on the SWITCH receipt. P5′o stays deferred behind W10 | §6.6, §7.3 |
...
| **DEC-50** | **THE INCREMENTAL INTEGRATION PROTOCOL** — a named, three-step procedure for admitting or changing any submodel: (1) train the submodel **alone** under phase-1 discipline against its own untrained baseline; (2) **retrain the interconnect** with it admitted (white matter A→B→C→D, regions frozen), gated on G2 + composed-metric improvement + non-zero attention mass to the new region, **else revert**; (3) **whole-mind unified training** under the monotone-improvement rule. **DEC-26's 2–5% cap is what makes step 2 cheap.** E0/E1/E2 are its first instance `[OP: csd-incremental-integration-protocol.md]`. **AMENDED 2026-09-07 — THE ATTENTION-MASS CLAUSE IS UNCHANGED; WHAT A PASS ON IT MEANS IS CORRECTED. A PASS IS NECESSARY, NOT SUFFICIENT.** *Nothing is weakened: G29's predicate, its `η/R` threshold, `η`, `R` and the **else revert** all stand exactly as ratified. The correction is to the reading of the result, and it is the reading this document used in four places.* **What the floor establishes:** attention mass was **allocated** to the new participant's slots — the read simplex did not collapse away from it. **What it does not establish:** that the participant's read carried information, or that the composite used what it read. **The two are decoupled on a stream with no recall dependency, and this is measured, not argued.** On phase A's synthetic stream (before the module spec's Amendment A4 made the store read query-dependent) the store read was a **constant** across items and batches, so its mutual information with the target was **exactly zero**, `dL/d(store attention) ≈ 0`, and the mass was an unidentified direction that random-walked. Measured: `mean(a_store)` = **0.0468 at 5, 8 and 64 primed records — identical to four decimal places — and 0.0488 with an EMPTY store**; emptying the store at evaluation moved dev loss by **~1e-4 nats** and dev `recall@1` **not at all**, while `mean(a_store)` ranged over **70x** (0.0043–0.3029); G29 fired on **2 of 8 seeds** and, at a fixed seed, flipped on **`OMP_NUM_THREADS` alone** (1/2/4/16 PASS, 8 FAIL); and the statistic is **non-monotone in steps** — 0.118 → 0.101 → 0.068 → **0.047 FAIL** → 0.201 → 0.329 — so training *longer* un-collapses it. **The gate is sound and the input was wrong:** run unchanged on a stream where the store is the **only** route to the label, the same predicate reads **0.191–0.640** across the whole trajectory and never fires, a 3.8x–12.8x margin. **The rule that follows, and it binds every future instance of step 2:** clause three may be read as evidence of admission only when the same receipt carries **independent evidence that the new participant's read is identified** — non-zero mutual information with the target, or a measured composed-metric effect from removing it. Absent that, the verdict on the run is **INCONCLUSIVE, not PASS**, and *"the store did not help"* and *"the store was never attended to"* remain the two different findings E2 already distinguishes. `[/akula-data/session-backup-staging/notes/GATE-DISCRIMINATION-2026-09-07.md; DECISIONS-AND-RENAMES-2026-09-07.md §1]` | §4.1, §6.4, §4.1 P5′ |
...
| **DEC-63** | **EPISODIC STORE GAP (a): DYNAMIC CAPACITY AND STALENESS DECAY — DEFAULT ADOPTED.** *Rationale: the operator asked for the contracts nailed down rather than deliberated, and gap (a) has an implementable default that E1's receipt then turns into a measurement.* `capacity_bytes(host, tick) = max(0, VRAM_total − KV_reserved(context_len, regions_active) − activation_reserve − safety_margin)`, a **pure function of live inputs, computed per host at scheduler-tick time and NEVER CACHED** — the 1080 Ti is preemptible and the active-region set changes under the store, so a capacity fixed at process start is not dynamic. **Staleness is an exponential half-life on `last_accessed`**, half-life a config constant starting at the order of the workspace context window in wall-clock time; the receipt **records which function ran**. **OPERATOR TO CONFIRM, two things and they are different questions:** the `safety_margin` value (recommend 2 GiB, the order of `hypha`'s display reserve), and **whether a residual claim is acceptable at all** given §9.11's finding that the residual may round to zero on the 5080 — the alternative being a **fixed floor reserved before the KV budget**, which trades context length for recall and is a product decision `[OP: csd-episodic-store-required.md]`. **AMENDED IN REVISION 3.5 BY DEC-70: `capacity_bytes` is a CEILING, NOT A TARGET.** The formula says what the store *may* claim; it never said what the store *should* claim, and a residual read as an allocation is how a frugal subsystem becomes the largest one on the card. The resident set is what the residency score admits **now**, bounded additionally by DEC-70's frugality cap, and `capacity_bytes` is the ceiling that bound may not cross `[OP: csd-memory-gate-overlays.md]`. **AMENDED 2026-09-07: `capacity_bytes` DECIDES PLACEMENT ONLY. MEMBERSHIP IS A DECLARED RECORD COUNT, AND SELECTION CARRIES NO RESIDENCY TERM.** *Three concepts, not two:* **placement** (where a record lives — MAY depend on the host, because it costs latency), **membership** (whether a record exists — must not), **selection** (which records a query returns — must not). *Measured defect:* E1's `_enforce_capacity` always ended in `_prune_disk()`, and the spill rate that feeds it follows `capacity_bytes`, which follows live VRAM — so a smaller card spilled more, reached the fixed `disk_max_items` at a smaller working set, and **hard-deleted rows a larger card still held**. Same input sequence, different membership, decided by the host, through the one path §1.3 clause (2) names as *"the only data-loss path in the store"*. The deletion **score** was already correct (importance only, GPU bonus deliberately excluded); the **trigger** was not. **The amendment, narrowly:** (i) the spill path and the deletion path are **unhooked** — `_enforce_placement` demotes and deletes nothing, `_enforce_membership` deletes and demotes nothing, and neither calls the other; (ii) deletion triggers on a **declared** ceiling (`TierBudget.max_records`) that no card can move, evaluated over **every** record rather than the spilled subset; (iii) that ceiling is **in RECORDS, not bytes** — a quantised model has narrower latents, so a byte budget would silently admit more records after quantisation, changing what the store answers out of a step that was meant to be behaviour-preserving, and re-quantisation is already a memory-gate migration trigger; (iv) **demote-on-VRAM-pressure is unchanged and stays** — DEC-70 ratifies it and it is correct, because demotion costs latency, not membership; (v) **the read score drops `gpu_resident_bonus` outright**, not to a tie-break — two hosts holding the same records must return the same read, and a tie-break still moves the returned set whenever relevance ties, which for mean-pooled `z_N` memories at max off-diagonal cosine 0.9993 is the normal case. **The latency the bonus bought moves to PREFETCH** — make the right records resident *before* the query and the read is equally cheap without changing the answer; a scheduler job, not built here. **Truncation at `b_store` stays:** a deterministic ranking cut at a fixed k is still deterministic. **The eviction score keeps the bonus**, correctly: it decides placement. **One residual path, recorded:** `promote` refreshes `last_accessed`, so a host that promotes more makes those records marginally less stale; `mark_gpu` does not. **Direction of travel, not licensed here:** hot/warm capacity derived from the *model* (a fixed record count as a ratio of model size) with cold unbounded and user-configured, which removes the host dependence at its source | §1.3, §8 gap (a), §4.1 E1, §4.1 E1a, §6.6, §9.11 |
...
| **DEC-78** | **THE FACULTY NAMING RULE: BY COGNITIVE FUNCTION, NEVER BY DOMAIN OR ANATOMY.** *Rationale: DEC-01 named the renamed `code` region `language_code`, folding its current corpus/battery domain into the id itself — exactly the pattern the operator's rule now forbids.* Operator ruling: *"regions are named by cognitive faculty and the function they provide, in accurate synthetic terms — never by knowledge domain (`code`) and never by colloquial anatomy. The region formerly called `code` is the LANGUAGE CENTRE: id `language`, with `specialisation: \"code\"` recorded as metadata"* `[OP, 2026-09-04]`. **Domains live as memory-gate personas (overlays) or as variant branches of the same Hub repo, never as region names.** **AMENDS DEC-01 IN PLACE**: canonical id `language`, `code` demoted from id to specialisation metadata; **DEC-03's `vl_latent` → `visual`** already satisfies the rule and needs no change. **Compatibility is unaffected**: everything on disk and on the Hub — receipts, matrix cell directories, variant ids, Hub branches/tags, corpus fingerprints and licence tiers — keeps the legacy names, resolved through the single alias module `cogsyndelta.regions.aliases` (`REGION_ALIASES = {"code": "language", "vl_latent": "visual"}`), which is the ONLY place the mapping lives; every reader resolves a region name through it before comparing, dispatching, or looking up config on it. **Disclosure of a resolved legacy name is a per-reader choice, not a second invariant this rule imposes on every reader alike**: `MindSpec.from_dict` (the interconnect ingest path) records it as a `region_alias_of` field with a `DeprecationWarning`; `cogsyndelta.cards`' Jinja templates print a **Faculty** line naming the canonical id and the legacy id the cell's receipts were recorded under; `scripts/csd-publish-checkpoint.py`'s hand-built card adds no such field, because its shape is pinned byte-for-byte against every already-published `tzervas/cogsyndelta-region-*` README (changing it changes what is already live on the Hub — see `docs/technical/model-card-pipeline.md`). **A writer that must keep an in-flight run's on-disk history stable does not canonicalize either**: `scripts/csd-train-all.py` deliberately writes `PretrainConfig.region`/the receipt `region` field as the caller's own spelling, because `cogsyndelta.regions.pretrain.pretrain_region` derives the checkpoint directory and receipt filename from that value, and canonicalizing it mid-run would split one region's history across two directories. **AMENDED AGAIN IN REVISION 3.6 (g11 fold of proposed DEC-82 — not a second DEC-78):** early-alpha compatibility is this **minimal alias table only**. Canonical ids: `language` (faculty; spoken "language centre" is **not** a third `name:` field) with `specialisation: "code"` as metadata — never `language_code` as the id — plus `visual`, `memory`, `reasoning`, `episodic_store`. **The only mapping in this tree:** `REGION_ALIASES = {"code": "language", "vl_latent": "visual"}` (`src/cogsyndelta/regions/aliases.py:40-43` `[V]`); `canonical_region()` at `:51-66` `[V]`. The runner accepts both spellings and points them at the **same** dict entry (`scripts/csd-train-all.py:527-548` for `REGIONS`, `:630-667` for `VL_REGIONS` `[V]`). **Refuse** dual live keys in the matrix. **No** forever `was:` JSON, no general translation layer, no `language_centre` catalogue id. Domains live as memory-gate personas (DEC-33 / DEC-81) or Hub **variant branches**, never `region-python`. `CSD-BRAIN-REGIONS.md` stays retired (DEC-11). **Falsifier:** a dataset-named region; dual matrix keys; donate-monorepo; `language_centre` as catalogue `name` | §1, §1.2, §1.4 |
| **DEC-79** | **AFFECT ISOLATION IS A BUFFERED REASON-OVER-EMOTION GATE, NOT A GLOBAL GAIN AND NOT A GOEMOTIONS REGION.** *Rationale: rev 3.5 declared affect twice — DEC-06 as a probe of a real faculty, §1.3 as a `[B,8]` broadcast gain on `a_r` — and neither sentence is the 2026-09-04 isolation contract.* **Keep DEC-06:** `classify_go_emotions` remains a **PROBE**, never a region (DEC-06 index row; §1.2 verdict row 7 `[V]`). **Amend §1.3 affect cell:** v1 shape is a **tagged** workspace stream `z_affect`, not anonymous dimensions and not a multiplicative gain on every tract including reason. **Reason weights are not conditioned on affect.** The decision integrator may read the tagged stream as **nuance** (salience, stake) within a bound. A different *logical conclusion* under affect is an isolation **FAIL**; a different *option among intact analyses* is the intended use and must remain optional via disconnect. **Loud disconnect** sets the affect channel to an explicit **zero gate** (not a learned null; integrator dropout so zero is in-distribution) and restores reason's own-task holdout within a stated tolerance — **bitwise CUDA equality is the wrong test**. **G0–G5 are unrun.** They are the measurement list when an affect row is promoted, not green gates. Phenomenal native-ness is **not a receipt**. Affect is **not** a workspace participant at v1 unless OD-20 says so (default **no**, so `R` is unchanged). Persona is not affect; values are not affect (DEC-80). **Evidence:** DEC-06 `:472,:582` `[V]`; §1.3 affect cell `[V]`; `rag://akula-csd-kb rank=3 score=0.6160 path=Experiments/g6-ternary-memory-gate-2026-09-04/S11-sapience-affect.md` `[V]`; `vault://akula-csd-kb/Experiments/g6-ternary-memory-gate-2026-09-04/S16-affect-isolation-gates.md` `[V]`. **Falsifier:** GoEmotions promoted on label-accuracy; untagged affect in reason's input; reason holdout moves on disconnect; affect is a dead channel. **What it amends:** §1.3 affect cell; does **not** reopen DEC-06 | §1.3, §8 OD-20 |
| **DEC-80** | **VALUES / MINDSET THAT MUST SURVIVE OVERLAY DROP ARE A BASE-TRAINING TARGET.** *Rationale: overlay unload restores **base weights**, not "no persona"; a basin that only lives in an overlay can be erased by the same SWITCH the safety story depends on.* **Keep DEC-59's measurement:** own CORPUS-CONTRACT entry, provenance, DEC-31 licence, held-out moral/safety probes vs **untrained and no-moral-corpus controls**; **not a toy-scale train row** (DEC-59 index row `[V]`). **Amend location and clock:** the default-mind moral basin is trained into **base** (P5′m remains the train row; declaration is now). Overlays hold droppable skills/personas. **PROCESS roles** (`train`/`probe`/`refuse`) bind (DEC-85). Freeze: no Jigsaw rater-pools as train (entire set **probe**); no wiki-talk-as-LM; no `rater_group` as desired behaviour; PD Stoic is **`held_seed`, `admitted: false`**. B1/H4 stay **discovery after H1**, not the next GPU increment (OD-22). **Evidence:** DEC-59 index row `[V]`; P5′m cell `[V]`; DEC-33/DEC-81 overlay drop restores trained base `[I]`. **Falsifier:** probe flat vs no-moral control; attack-string NLL as target; demographic identity as desired behaviour; overlay-only morals claimed as default-mind values after drop. **What it amends:** DEC-59 (location/declaration now; train clock unchanged); pairs with DEC-81 | §5.5, §4.1 P5′m, §8 OD-22 |
| **DEC-81** | **OVERLAY LOAD IS THE UNFREEZE; COMPOSE AT MATMUL TIME; UNLOAD RESTORES BASE BIT-EXACTLY ON THE SWITCH RECEIPT.** *Rationale: DEC-33 already required loud disconnect; 2026-09-04 measured the smallest honest test of load≠merge.* **Base faculty weights stay frozen** on the persona path. "Unfreeze" **is the overlay load**, not autograd on base W. Compose `W+ΔW` at `F.linear` (or equivalent); **`param.data +=` is a constructed FAIL**. A delta that is attached but never read is a constructed FAIL (dead switch). **Measured `[V*]`** `g6-switch-toy/REPORT.md` + `receipt.json`: default **PASS** (max\|y1−y0\|=1.032e-01, cos=0.999663758, h2==h0, y2 bytes==y0); `--forbidden-inplace` **FAIL** restore; `--dead-switch` **FAIL** divergence; CSD pin `fb7a1bb8f540`; class `ResidualMLPRegion(dim=16, hidden_dim=32)` CPU. **Does not prove** persona, Memory-Gate, affect, 16M faculty, GPU bit-exact, learned deltas. **Do not write SWITCH files into CogSynDelta** unless a later test PR. **Do not pull full P5′o into E1** (DEC-66). Overlay drop restores **trained base**, not an empty mind. **Evidence:** DEC-33 index row `[V]`; DEC-70 index row `[V]`; P5′o cell `[V]`; SWITCH REPORT `[V*]`. **Falsifier:** inplace merge that cannot restore; dead switch PASSing; marketing the 16-d toy as a persona. **What it amends:** DEC-33 / P5′o gates (receipt shape); not DEC-49's store build | §6.6, §4.1 P5′o |
| **DEC-82** | **MULTI-AXIS TEMPORAL GROUNDING (τ) IS A DECLARED DISCOVERY SEAM PAIRED WITH STREAMING LATENT MODE; NOT A REGION AND NOT A PHASE-2 GPU ROW.** *Rationale: DEC-47 already says the scheduler's iteration depth IS the latent-reasoning loop; the operator's version axis is a VSA/FPE pool over versions of one concept, not wall-clock step index and not k KV copies.* **Streaming latent mode** is a **runtime/scheduler policy** under DEC-47: keep writing latents without re-tokenising; **no** `temporal` / `streaming` faculty. **no source found** for a streaming flag in `src/` this pass `[V-abs, grep streaming/temporal faculty in src/]`. **τ:** one shared head field, concept-addressed, FPE \(z(\tau)=z^{\tau}\), reserved pool \(O(D)\) not \(O(kD)\); interference past ~\(D/10\), not allocation failure. Algebra is operator/backlog **`[I]`** until a PROTOCOL exists. **Not W7v, not phase D, not extra all-to-all, not per-token unique graphs.** Differential Transformer subtract-to-isolate is **not** versioning. Pair with DEC-47: successive latent states may *be* the τ axis; streaming writes FPE-tagged diffs when the store exists. **blocked_by:** writable store (E1) + H1. **Evidence:** DEC-47 `[V]`; DEC-53 `[V]`; §6.8 `n_iter` is explicitly not recurrent-depth `[V]`. **Falsifier:** \(k\) materialised copies required; unbind fails at small \(N\); implemented as a new region; decode-then-re-encode between regions. **What it amends:** DEC-47 (streaming sentence); §6.8/§6.9 (do not silently retie weights); no new §4.1 GPU row | §2.2, §6.8, §8 OD-25 |
| **DEC-83** | **W7v-cfg (CONFIG HALF) IS ON `main`; VISUAL RETRAIN REMAINS BLOCKED ON OD-4; DO NOT WARM-START THE UNRELEASABLE 64-px CHECKPOINT.** *Rationale: g8's first build landed; the W7v cell still reads like a branch.* **Merged:** `8fbd25b` PR #36 2026-09-04 `[V]` git (ancestor of HEAD `7867606`). **Defaults:** `JEPAConfig.image_size = FRAME_SIZE` (128), `patch_size = PATCH_SIZE` (8), `N_PATCHES = 256` (`src/cogsyndelta/vl/composite.py:51-60` `[V]`; `src/cogsyndelta/model/vl_jepa.py:47-75` `[V]`). Positions are **2-D sincos recomputed** for `cfg.grid` (`sincos2d_pos_embed`), never interpolation of the length-64 1-D table. **`VL_REGIONS["visual"]["corpus_source"]` is `None`** (`scripts/csd-train-all.py:630-644` `[V]`); `run_vl_region` refuses to start (`:1254-1266` `[V]`). `IJEPA.encode` is `src/cogsyndelta/model/vl_jepa.py:581-587` `[V]` (target-encoder embed). **Not done:** GPU retrain; OD-4 mix pin; visual matrix row; v2-stamped VL train envelope. A4 0.0606/0.2625 remain **pre-rebuild tiny-imagenet / cifar100** numbers and are **void as post-OD-4 gates**. Do **not** init from `step-8000.pt`. `stream_vae` / `residual` stay non-faculties (DEC-08 / DEC-07). **Amend W7v leftover copy:** §5.4 live arithmetic is **six bins / 3,072 cross-faculty eval / ten pairs**; "2 of 4 = 1,024 of 2,048" is a row leftover. **Evidence:** git `8fbd25b` `[V]`; files above `[V]`; OD-4 `[V]`. **Falsifier:** default `n_patches ≠ 256`; first GPU cell globs tiny-imagenet; card prints 0.0606 as post-OD-4. **What it amends:** W7v status cell; §5.4 leftover sentence; encode line cites | §4.1 W7v, §5.4, §8 OD-4 |
| **DEC-84** | **`csd-metrics/v2` IS THE SHIPPED SCHEMA; `compare()` IS THE REFUSE-FUNCTION.** *Rationale: g7 specified it; g8 recorded the gap; the code now exists.* **Stamp:** `METRICS_SCHEMA_V2 = "csd-metrics/v2"` (`src/cogsyndelta/regions/_receipt.py:57,206` `[V]`). Envelope remains `SCHEMA = "model-pipeline-receipt/v1"` (`src/cogsyndelta/pipeline/receipt.py:38-47` `[V]`). **`compare()`** refuses unless `metrics_schema`, `corpus.fingerprint`+`fingerprint_scheme`, `battery_id`, `k`, `pooling`, `checkpoint_sha256`, `region`, `git_sha`, `seed` match (`src/cogsyndelta/eval/metrics.py:691-716,765-845` `[V]`). No blended quality score. `assert_sameness()` (`:848` `[V]`) stays the only blessed cross-battery path. v2 is **naming/discipline, not a formula rewrite** (`docs/design/METRICS-METHODOLOGY.md:10-20` `[V]`). **Merges:** metrics PR **`86532f2`**; later HEAD `577c8eb` (model-cards) **contains** v2 as ancestor; current HEAD `7867606` still does. **Retract** g8 "compare() has no identity predicate" as present-tense. Dual harness unchanged: native receipts are gates; industry JSON in `detail.external`; no invented MIEB. **Evidence:** files above `[V]`; git `86532f2` `[V]`. **Falsifier:** silent cross-identity diff; blended score; invented public MTEB/MIEB; PR vs entropy as one gate. **What it amends:** methodology pointer as **shipped**; g8 leftover; first post-OD-4 VL receipt must carry v2 identity (DEC-83) | MM, `metrics.py` |
| **DEC-85** | **PROCESS ROLES BIND ON FACTORY EMITS; ADMIT IS NOT A TRAIN GLOB.** *Rationale: DEC-57 loop existed; 2026-09-04 showed ADMIT and train-config diverging.* Every emit row is exactly one of `train` / `probe` / `refuse`. Stamp `operations[]`. **Admission 2026-09-04:** 8/8 ADMIT, policy `2026-09-03.r3`, 0 quarantined `[V*]` `admission-2026-09-04/REPORT.md`. **DO-NOT-ADMIT untouched:** pd-stoic-v0, jigsaw rater-pools, wiki-talk, recyclable-waste. **tinyquickdraw ADMIT ≠ Mix B train** (probe-only; B1 share ~0.66–0.80 vs 0.40). **cpc-codes** ADMIT as language titles, "not a code corpus". **Adapter:** 7/8 "no declared tier" until faculty→region map (OD-24). **arXiv `grant_scope`:** `whole_corpus` on disk; `metadata_only` would REFUSE (OD-23). DEC-73 percents unchanged; visual 0.3% still the **64-token** footnote unless restated for 256 patches. **Evidence:** DEC-57/73 `[V]`; PROCESS `[V*]`. **Falsifier:** probe leaked to train; rater_group SFT; adapter guesses a tier; SYN-L3 launders BLOCKING. **What it amends:** DEC-57 emit contract; DEC-73 wiring; not DEC-31 | §5.8, §4.1 P2′f, §8 OD-23/OD-24 |
| **DEC-86** | **HARNESS REPOS STAY SPLIT; EARLY-ALPHA PUBLISHING USES THE SAME MINIMAL ALIAS; DO NOT DONATE CSD.** *Rationale: DEC-75 already split model vs tool; the workflow library now has a home.* CogSynDelta keeps the model. **`tzervas/gpu-pack`** = W7p (DEC-75 `[V]`). **`tzervas/csd-autodev`** = autodev spec rev 5 `ece346b` (DEC-76 `[V]`). **`tzervas/agent-workflows`** = workflow library + the g11 research pack. **Do not** mint a second workflow repo. **Do not donate** the CSD monorepo. Hub: faculty-shaped names after DEC-78; **OD-19** whether to keep `cogsyndelta-vl-jepa` or mint `region-visual`; **do not** upload `step-8000.pt`. PRE-1/2/3 still **block autodev GPU** unless separately measured closed (**no source found** this pass). SWITCH stays out of CSD until a test PR (DEC-81). **Evidence:** DEC-75/76 `[V]`. **Falsifier:** workflow duplication; autodev GPU while PRE-* red; dual live Hub region keys. **What it amends:** DEC-75 pointer list; publishing side of DEC-78 | §6.7, §8 |
...
the pattern: a decision that stops being true is amended in place and keeps its number. **Revision 3.5 appends DEC-68 to DEC-77, also in numeric order, and renumbers nothing**; DEC-56 and DEC-63 are amended in place by DEC-73 and DEC-70 and keep their numbers and their original text. **Revision 3.6 first appended DEC-78** (faculty naming) and amended DEC-01 and DEC-44 in place. **This fold amends DEC-78 in place again** (early-alpha minimal alias; g11's proposed DEC-82 is not a second DEC-78) **and appends DEC-79 to DEC-86 in numeric order**; DEC-06, DEC-33, DEC-47, DEC-57, DEC-59 and DEC-75 keep their numbers and their original text with the amendment cited alongside it, the same pattern as DEC-56/DEC-63. **2026-09-07 amends DEC-63 in place a second time** (`capacity_bytes` decides placement, never membership and never selection; the membership ceiling is a declared record count and the read score carries no residency term) **and mints no new number**: a decision that stops being wholly true keeps its number and gains an amendment, and a second amendment to one decision is still that pattern rather than a reason to renumber.
...
### DEC-78 — the alias table, and why it is the only compatibility that exists

Early-alpha compatibility is **this table only** (g11 fold into DEC-78). The runner may accept
a legacy spelling and immediately share the same dict entry; it may not keep two live keys.

| legacy (on disk / Hub / receipts) | canonical id | in `REGION_ALIASES`? |
|---|---|---|
| `code` | `language` | **yes** — `src/cogsyndelta/regions/aliases.py:40-43` `[V]` |
| `vl_latent` | `visual` | **yes** — same dict `[V]` |
| `reason` | `reasoning` | **no** — DEC-04 is paper; `config/mind/csd-regions.json` still lists `reason` `[V]` |

Spoken "language centre" is the same faculty as id `language`. It is **not** a third `name:`
field and must not appear as a catalogue id. `specialisation: "code"` is metadata on
`language`, never part of the id. Domains (`python`, `biology`, banking intents) live as
memory-gate personas or Hub variant branches, never as region names. **Refuse** dual live
keys in the matrix. **No** forever `was:` JSON and no general translation layer.
...
| **affect** | **PLACEHOLDER, unusual shape**: `emits: "global"`. **AMENDED IN REVISION 3.6 BY DEC-79** | **Original v1 sketch (kept as history, not the isolation contract):** a `[B,8]` gain vector broadcast onto every tract — a multiplicative modulation of `a_r`, **not** a participant in the attention. **The isolation contract, which is a different machine:** a **tagged** workspace stream `z_affect` the decision integrator may read as nuance (salience, stake) within a bound; **reason weights are not conditioned on it**. A different *logical conclusion* under affect is an isolation **FAIL**; a different *option among intact analyses* is the intended use and must remain optional via disconnect. **Loud disconnect** sets the affect channel to an explicit **zero gate** (integrator dropout so zero is in-distribution — not a learned null) and restores reason's own-task holdout within a stated tolerance; bitwise CUDA equality is the wrong test. `classify_go_emotions` stays a **PROBE** (DEC-06). G0–G5 are **unrun** and are the measurement list when an affect row is promoted, not green gates. Default: **not** a workspace participant at v1 (OD-20), so `R` is unchanged. Persona is not affect; values are not affect (DEC-80). |
...
### DEC-79 — isolation is the tagged channel, not the `[B,8]` gain

The `[B,8]` broadcast-gain sketch in the affect cell above is **kept as the original v1
placeholder**, not as the 2026-09-04 isolation contract. A global multiply on `a_r` *does*
change reason's attention mass, so isolation would have to be proved *after* the multiply.
The tagged stream plus an explicit zero-gate can set affect to exactly zero on disconnect
without rewriting reason weights. **G0–G5 remain unrun** — name them when an affect row is
promoted; do not print them green. Promoting affect to a workspace participant is OD-20 and
would recount §5.4; the silent default is **no**.
...
The v1 participant list is **five** participants — `language`, `memory`, `reasoning`
...
1. **10,712,448 of `visual`'s 22,905,216 params do not ship** [V\*] — and revision 1 named
...
Schedule {
  ...
  "nodes": [
    ...
    { "region": "language",      "active": true,  "depth": 1, "intensity": 0.94, "priority": 1,
      "context_tokens": 96, "read_tokens": 64, "precision": "int3", "resident": true,
  ...
  "edges": [ {"src":"visual","dst":"language","weight":0.41},
             {"src":"memory","dst":"language","weight":0.22} ],
  "stages": [ ["visual","memory"], ["language"] ],
  "lockstep_groups": [],
  "halt_at": 2,
...
| **G2** no composition tax | composed ≥ each faculty's own single-bin score − 1 point | paired, per bin, in DEC-41's metric. **`episodic_store` is EXEMPT and the exemption is declared rather than assumed** `[DEC-49]`: it is non-parametric and has no standalone task, so it has no own-bin score to regress against, and a gate applied to a quantity that does not exist is the `residual_mlp` defect in gate form. **Its substitute is E2's gate** — the composed metric improves and the store draws non-zero attention mass — which is a stricter test than G2 because G2 only forbids harm while E2 requires benefit. **AMENDED 2026-09-07 — WHICH CLAUSE CARRIES THE "REQUIRES BENEFIT" CLAIM.** *The exemption stands, E2's gate is unchanged, and so is this row's conclusion; what is corrected is which half of E2 earns it.* **The benefit is required by E2 clause (ii), the composed-metric improvement — not by the attention-mass clause.** Measured 2026-09-07, `mean(a_store)` and the store's mutual information with the target are **decoupled**: the floor was cleared on a stream whose store read was a constant, MI exactly zero, and emptying the store moved dev loss by ~1e-4 nats and dev `recall@1` not at all while the mass moved over 70x. Non-zero attention mass therefore shows that mass was **allocated**, never that the store **contributed** — see DEC-50's 2026-09-07 amendment for the measurement and the rule it fixes |
...
`memory` merge, designed as the first token-aware retrain), then **W7a** (`language`,
...
> by changing `language`'s **weights** — which, under the frozen-trunk reading its own row
...
| **P5′o** | **Memory-gate overlays** — the dynamic-paging seam's first client (DEC-33, §6.6) `[OP: csd-memory-gate-overlays]`. **AMENDED IN REVISION 3.6 BY DEC-81:** compose `W+ΔW` at matmul time; `param.data +=` is a constructed FAIL; a dead switch is a constructed FAIL. | **An overlay applied and then disconnected reproduces the base model's receipt metrics EXACTLY, and the disconnect is logged** — SWITCH-shaped: unload restores base bit-exactly on the SWITCH receipt `[DEC-81]`. Plus: an overlay **refuses to attach** to a base checkpoint fingerprint it was not trained against (verified by constructing the mismatch); tiered-residency policy reports hit rate, page-in latency and VRAM held. The 16-d CPU ResidualMLP PASS does **not** close this row. | W10 | deferred | 5080 `.251` |
...
2. **RETRAIN THE INTERCONNECT** with the new region admitted: white matter **A (dense) → B (distil)
   ...
   **AMENDED 2026-09-07 — the third gate is unchanged and it proves less than this sentence implies.**
   Non-zero attention mass shows that mass was **allocated** to the new region; it does not show
   that the region's read carried information, nor that the composite used it. Measured, the two
   are decoupled: the floor was cleared on a stream whose store read was a constant with mutual
   information exactly zero, and emptying the store moved dev loss by ~1e-4 nats and dev `recall@1`
   not at all while `mean(a_store)` moved over 70x. **The gate still fires correctly** — on a
   stream where the store is the only route to the label the same predicate reads 0.191–0.640 —
   so the threshold and the revert stand; a pass on it is **necessary, not sufficient**, and needs
   independent evidence that the region's read is identified before it certifies admission.
   The measurement and the rule are at **DEC-50, amended 2026-09-07**.
...
| W7a `language`, `reasoning` | 2 (+2 allowance) | text regions, three loss terms | ~2,000 s [I] |
...
language` cross-faculty bin. Once a region trains on them they are gone forever.
...
`language` can solve and assert rejection; construct a general-bin row and assert it is
...
|---|---|---|
| per-faculty eval — `language`, `memory`, `reasoning`, `visual`, `affect`(probe), `general` × 512 | 3,072 | 6 bins, not 7. `general`'s target is DEC-41's `NULL` candidate. **`episodic_store` gets NO per-faculty bin** — it has no standalone task, which is also why it is exempt from G2 (§2.7.2) `[DEC-49]` |
...
|---|---|---|---|
| `memory × language` | given a natural-language issue, retrieve the API description, then find the function implementing it | **live** | the old `retrieve × code` |
...
| `visual × language` | locate an element in a frame, then name or act on it | **BLOCKED until W7v** `[A4 fixed]` | the old `vl × classify`. Unconstructible against 64×64 single-object tiles, and the encoder cannot ingest a larger frame at all |
...
`language × reasoning` and `visual × reasoning` — had **no items that require both regions**;
...
|---|---|---:|---|---|
| **X1** issue → API description → function | `memory × language` | 256 | §5.5(a) retrieval half | {memory, language} |
| **X2** **executable three-way join** — retrieve the matching problem (**memory**), then rank candidate solutions by whether they pass the tests (**language** + **reasoning**), distractors are other problems' correct solutions | `memory × language` | 256 | §5.5(a), label = sandboxed exit code | {memory, language}, **{language, reasoning}**, {memory, reasoning} |
...
| **X4** composite locate-then-name | `visual × language` | 512 | §5.5(b), needs **W3r** + W7v | {visual, language} |
...
|---|---|---:|---|
| {memory, language} | X1 + X2 | **256** | live |
| {memory, reasoning} | X2 + X3 | **384** | live |
| {language, reasoning} | X2 | **128** | live, **thin** |
| {visual, language} | X4 | **256** | blocked until W3r + W7v |
...
| {episodic_store, language} | X7 | **256** | blocked until **E1 + the episode harness** |
...
> compose eval from 6,144 to **5,120**, `R` from 5 to **4** (`language`, `memory`,
...
> under revision 2 it was not** `[N2 fixed]`: {memory, language} = X1+X2, {memory, reasoning}
> = X2+X3, **{language, reasoning} = X2** (the executable three-way join, with a subprocess
> exit code for a label — this is the pair that had no shape and whose true null rate was
> therefore **0**, not 0.125), {episodic_store, memory} = X7, {episodic_store, language} = X7,
...
> under the slip, **X2 takes the whole of the `memory × language` bin (512 eval / 256
> sealed) and X1 moves to train-only**, giving {language, reasoning} **≥256 sealed items**
> like the other five.
>
...
5,120, `R` to **4** (`language`, `memory`, `reasoning`, `visual`), the pairs to **6**, and the
...
> rank candidate solutions by whether they pass the tests (**language** + **reasoning**),
...
{memory, language} (the retrieval step), **{language, reasoning}** (deriving the
constraint the statement implies is `reasoning`; reading whether a candidate program satisfies it
is `language`; neither alone ranks the candidates) and {memory, reasoning}. It is the shape
...
two cross-faculty item shapes it makes constructible — `auditory × language` and
...
problem #4,117. Nothing stops `language` from later training on problem #4,117: its hash is
...
3. **Verify by making it fire, on the derived case.** Build a composite from a row already in
   `language`'s realised training set and assert the training run **refuses to start**. A
...
| object | term | why |
|---|---|---|
| `language`, `reasoning` | MIT | no restrictive input |
...
### DEC-85 — PROCESS roles bind on emits; ADMIT is not a train glob

DEC-57's loop is unchanged. What 2026-09-04 added is an operational **role** on every emit:
exactly one of `train` / `probe` / `refuse`, stamped on `operations[]`. **ADMIT ≠ train glob.**
The 2026-09-04 admission round was 8/8 ADMIT under policy `2026-09-03.r3` `[V*]` — tinyquickdraw
is ATTRIBUTION and ADMIT and still **probe-only** for Mix B (B1 share over cap); cpc-codes
ADMIT as language **titles**, "not a code corpus"; PD Stoic stays `held_seed`, `admitted: false`.
The admit adapter reported 7/8 "no declared tier" because faculty names ≠ region names — that
wiring is OD-24 and binds to DEC-78 ids, **not guessed**. arXiv `grant_scope` is OD-23. DEC-73's
percents are **not** re-averaged here; visual 0.3% still carries the 64-token footnote unless
restated for 256 patches.
...
4. **Disconnect is LOUD.** Reverting to base weights is observable and logged, never a silent
   ...
   appears in the log.** **AMENDED IN REVISION 3.6 BY DEC-81:** that "exactly" is SWITCH-shaped —
   compose `W+ΔW` at matmul time; **`param.data +=` is a constructed FAIL**; a delta attached
   but never read is a constructed FAIL (dead switch); unload restores base **bit-exactly on
   the SWITCH receipt**. The 16-d CPU ResidualMLP PASS does **not** prove a 16M persona.
...
### DEC-81 — compose at matmul time; unload restores base on the SWITCH receipt

"Unfreeze" on the persona path **is the overlay load**, not autograd on base W. Base faculty
weights stay frozen. The existence proof `[V*]` is `g6-switch-toy/REPORT.md`: default PASS
(max\|y1−y0\|=1.032e-01, cos=0.999663758, h2==h0, y2 bytes==y0); `--forbidden-inplace` FAIL
restore; `--dead-switch` FAIL divergence; class `ResidualMLPRegion(dim=16, hidden_dim=32)`
CPU; CSD pin `fb7a1bb8f540`. **Do not write SWITCH files into this tree** unless a later test
PR. **Do not pull full P5′o into E1** (DEC-66). Overlay drop restores **trained base**, which
is why values that must survive drop are a base-training target (DEC-80).
...
**DEC-63 IS AMENDED AGAIN, 2026-09-07, AND THE TWO AMENDMENTS SAY DIFFERENT THINGS.** The
revision-3.5 amendment above bounds *how much* the store may claim. This one bounds *what
`capacity_bytes` is allowed to decide at all*: **placement, never membership.**

The defect was measured in E1's code, not argued from the text. `_enforce_capacity` always ended
in `_prune_disk()`, and the spill rate feeding that prune follows `capacity_bytes`, which follows
live VRAM. So a smaller card spilled more, reached the fixed `disk_max_items` at a smaller working
set, and **hard-deleted rows a larger card still held** — the same input sequence producing a
different store depending on which card ran it, through the one path §1.3 clause (2) names as *"the
only data-loss path in the store"*. Worth being precise about where the fault was: the deletion
**score** was already right — importance only, with the GPU bonus deliberately excluded and the
reason recorded — so nothing about *which* row loses was host-dependent. **The trigger was.**

| | decides | may depend on the host | mechanism |
| --- | --- | --- | --- |
| **Placement** | where a span sits | **yes**, and should — a dynamic capacity is the point | `_enforce_placement`, bounded by `capacity_bytes`; scored by `_score_meta` |
| **Membership** | whether a record exists | **no** | `_enforce_membership`, bounded by `TierBudget.max_records` |
| **Selection** | which records a query returns | **no** | `_rank_key`, scored by `_read_score` |

Five clauses, and the third and fifth are the ones most easily got wrong:

1. **The two paths are unhooked.** `_enforce_placement` demotes and deletes nothing;
   `_enforce_membership` deletes and demotes nothing; neither calls the other. `learn` and
   `evict` invoke both, membership first.
2. **Deletion triggers on a declared ceiling**, evaluated over **every** record rather than over
   the spilled subset. A declared number that is still derived from live VRAM would reintroduce
   the bug under a new name, so the ceiling is configuration, not measurement.
3. **The ceiling is in RECORDS, not bytes.** A quantised model has narrower latents, so a byte
   budget silently admits *more* records after quantisation — changing what the store holds, and
   therefore what it answers, out of a step that was supposed to be behaviour-preserving.
   Re-quantisation is already a memory-gate migration trigger; it must not also move membership.
4. **Demote-on-VRAM-pressure stays**, unchanged. DEC-70 ratifies it and it is correct: demotion
   costs latency, not membership. Removing disk-overflow deletion entirely would have been the
   wrong repair.
5. **The read score drops `gpu_resident_bonus` outright.** Making membership host-invariant is
   not enough on its own: two hosts holding *exactly the same records* still returned different
   reads, because the read ranked on the eviction score and that score prefers whatever is
   already on the card. The bonus is a placement term that had leaked into selection, where it
   changes the answer rather than the latency. It is **removed, not demoted to a tie-break** — a
   tie-break still moves the returned set whenever relevance ties, and for mean-pooled `z_N`
   memories at a measured maximum off-diagonal cosine of 0.9993, ties are the normal case.
   **Truncation at `b_store` stays**: a deterministic ranking cut at a fixed *k* is still
   deterministic, and the budget was never the problem. **The eviction score keeps the bonus**,
   correctly — it decides placement, which is exactly what a residency term is for.

**Where the latency goes.** Preferring a resident record bought a cheap read by changing the
answer. Making the *right* records resident **before** the query buys the same cheap read without
touching what comes back, so the benefit moves to **prefetch** rather than disappearing. That is a
scheduler's job and is not built by this amendment.

**One residual path, recorded rather than quietly closed:** `promote` refreshes `last_accessed`,
so a host that promotes more often makes those records marginally less stale and therefore
marginally better-ranked; `mark_gpu` does not. Closing it needs a decision about whether a
promotion should count as an access at all, which is a different question from this one.

**Direction of travel, recorded but not licensed here:** hot/warm capacity derived from the
*model* — a fixed record count as a ratio of model size, with cold unbounded and user-configured —
which removes the host dependence at its source rather than breaking one propagation path.
...
DEC-76). **AMENDED IN REVISION 3.6 BY DEC-86:** `tzervas/agent-workflows` is the workflow-library
home of the g11 research pack. **Do not** mint a second workflow repo. **Do not donate** the CSD
monorepo. Early-alpha publishing uses DEC-78's same minimal alias (OD-19). SWITCH stays out of
this tree until a test PR (DEC-81). PRE-1/2/3 still block autodev GPU unless separately closed.
...
### DEC-82 — τ is a declared seam; streaming is a scheduler policy; do not silently retie weights

**Streaming latent mode** is not a region and not a flag found in `src/` this pass
`[V-abs, grep]`. It is a **runtime/scheduler policy** under DEC-47: keep writing latents
without re-tokenising. **τ** is a reserved FPE pool over versions of one concept, \(O(D)\)
not \(O(kD)\), discovery, **not W7v, not phase D, not a §4.1 GPU row** (OD-25). Pair: successive
latent states may *be* the τ axis; streaming writes FPE-tagged diffs **when the store exists**.
`blocked_by`: E1 + H1. **Do not silently retie weights** to invent a temporal faculty.
...
### DEC-84 — `csd-metrics/v2` is shipped; `compare()` is the refuse-function

`METRICS_SCHEMA_V2 = "csd-metrics/v2"` (`src/cogsyndelta/regions/_receipt.py:57,206` `[V]`).
The envelope stays `SCHEMA = "model-pipeline-receipt/v1"`
(`src/cogsyndelta/pipeline/receipt.py:38-47` `[V]`). `compare()`
(`src/cogsyndelta/eval/metrics.py:765-845` `[V]`) refuses unless every `MetricIdentity` field
matches (`:691-716` `[V]`). `assert_sameness()` (`:848` `[V]`) is the only blessed
cross-battery path. v2 is naming and comparison discipline, not a formula rewrite
(`docs/design/METRICS-METHODOLOGY.md:10-20` `[V]`). Merged `86532f2` (PR #33). Retract g8's
"compare() has no identity predicate" as present-tense. First post-OD-4 VL receipt must carry
the v2 identity (DEC-83).

### DEC-86 — harness repos stay split; do not donate CSD

Pointer list beside DEC-75: `tzervas/gpu-pack` (W7p), `tzervas/csd-autodev` (rev 5 `ece346b`),
`tzervas/agent-workflows` (this research pack). **Do not** mint a second workflow repo. **Do
not donate** the CSD monorepo. Early-alpha publishing uses DEC-78's minimal alias. Hub visual
slot is OD-19; do not upload `step-8000.pt`. PRE-1/2/3 still block autodev GPU unless
separately closed. SWITCH stays out of this tree until a test PR (DEC-81).

## Operator asks added in revision 3.6 (OD-19 to OD-25)

Still open from rev 3.5 (not restated as new numbers): **OD-4** visual corpus replace-at-W7v;
**OD-16** two `csd-lab-console` copies; **OD-17** W4 pivot vs amend; **OD-18** five legal
readings — item 1's card clause needs no lawyer and should be done without waiting.

| # | decision | recommendation if silent |
|---|---|---|
| **OD-19** | **Hub visual slot:** keep private `tzervas/cogsyndelta-vl-jepa` as the future `visual` card, or mint `tzervas/cogsyndelta-region-visual` after the runner rename so yaml `{region}` matches. Both options **refuse** uploading `step-8000.pt`. Public 404 this programme ≠ published weights. The runner rename **has** landed on this tree (`7867606`, DEC-78). | **Keep `vl-jepa`**; do not mint a fourth empty repo. Archive `region-stream-vae` / `region-residual` privately with honest `weights: none` cards citing DEC-08 / DEC-07. |
| **OD-20** | **Is `affect` a workspace participant** (changes `R`, ablation pairs, G3′), or a tagged non-participant stream (DEC-79 default)? | **Non-participant at v1.** Isolation first. Promoting to participant is a later OD with a full §5.4 recount. |
| **OD-21** | **Store `scope` axis** (principal / session / persona-basin). Already "operator to confirm" on DEC-64. Listed so 3.6 does not pretend it closed. | **Principal**, session as a server-bounded sub-segment (DEC-64 default). E1 not blocked. |
| **OD-22** | **Promote B1/H4 moral train now** (PD Stoic + operator MIT as base seed) or wait? | **Wait.** Declare DEC-80 now; train after H1; PD Stoic stays `held_seed`. |
| **OD-23** | **arXiv landing `grant_scope`:** `whole_corpus` (on disk) vs `metadata_only` (factory REFUSE). Admission REPORT flagged, not resolved. | Do not silently flip. If metadata-only is the reading, the landing is not train. |
| **OD-24** | **Faculty → CSD region map** for `csd-corpus-admit.py` (`language_trunk` / `language_code` / `visual` currently "no declared tier"; `cpc-codes` may not be the programming-code region). | Bind to DEC-78 names. **Do not guess.** `language`←cpc titles is not an obvious yes. |
| **OD-25** | **Promote τ / FPE reserved pool** (B3) to a numbered §4.1 row now? | **No.** DEC-82 seam only. After E1 + H1. |
...
--- COMPOSED MIND v1 ----------------------------------------------------------
language + memory + reasoning + visual                58,907,264      [V*]
+ white matter (incl. the store's two projections)    27,424,039      [I]
