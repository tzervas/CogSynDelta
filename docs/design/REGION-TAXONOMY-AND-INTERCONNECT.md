...
>
> **THE HARNESS NOW EXISTS (2026-09-07)** — `src/cogsyndelta/eval/episode_harness.py`, run over
> the whole X7 eval shard at `docs/design/evidence/w3-episode-harness-2026-09-07/`. Turn 1's write
> commits through `learn()` on `EpisodicStoreImpl`/`SqliteBackend` and is read back before turn 2
> runs; the partition is a fresh `Scope` session per episode; and turn 2's step-8 read is
> instrumented, so every episode records whether it actually returned turn 1's record (2,048 of
> 2,048 did). **This does NOT move the pairs from `NOT SCORED` to scored**, and the distinction is
> the point: the harness removes the *mechanical* blocker, while admission (NSRS `s_r`, GPU-blocked
> and blocked on W2b) and a token-aware `language`/`memory` retrain (W1) still stand between a
> runnable episode and a reportable number. The slip branch above is unchanged until those clear.
