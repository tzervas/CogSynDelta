...
description: Steer the autodev loop then HTTP-apply one allowlisted worktree path. No SSH. No kang-main-wip.
---

# steer-apply-worktree

Identity **autodev**. Apply Bearer is `csd/apply-token` in the CSD vault
(`/akula-data/cabal/csd-apply.token`). Never print tokens. Never SSH.
Never write `kang-main-wip` / `python-ai/memory-gate`. Prefixes only:
`src/`, `tests/`, `docs/`.
...
`worktree` allowlist: lab keys `p1-08`, `p1-07` (not `kang-main-wip`).
