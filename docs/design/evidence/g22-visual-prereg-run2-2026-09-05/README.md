...
quantize stages; #52 pin bump) with the same corpus, config and seeds. Graded from the receipts
in this directory, never from the run log.
...
| code | pin `run.code.sha` = `3ce18db` (the #51 merge); checkout main `6964c0a` (the #52 merge); the only drift between them is `program/matrix/csd-matrix.yaml` (the pin line itself) |
| corpus | `visual-clean-v1` (Mix B), fingerprint `ab5761b65714e4ba4d7c36df095f3599`, 581,280 train images |
| config | W7v-cfg (128 px / patch 8 / dim 384 / depth 6 / 6 heads / predictor 192×3), steps 4000, batch 128 |
...
## H1 (PREREG §1): EuroSAT-test pooled linear probe, threshold `max(untrained, 0.1) + 0.01`, `collapsed = false`, both seeds
...
F1, F3, F4 not triggered on either seed (untrained baseline written before step 1, `resumed = false`).

**H1: PASS on both seeds.** Replication against run 1: seed 0 0.7191 → 0.7224, seed 1 0.7298 → 0.7237
(same seeds, same corpus, same config; CUDA training is not bitwise, so a few tenths of a point of
spread is expected).

## Transfer (Fashion-MNIST t10k, n_eval 2000, named in the receipt as `fashion-t10k` / `zalando/fashion-mnist`)
...
- Quantize packs the deployed EMA target encoder only (`target_encoder.*`, 10,712,448 params,
  42,849,792 fp32 bytes → 4,265,472 stored); the training-only context encoder and predictor are
  never packed. 27 probe evaluations per plan, about 6 minutes per cell.
...
quantization metric. Before 3-bit is trusted as a shipped width, check a more sensitive
read-out (token-surface probe, or the transfer set's drop) against the same plan. Filed as a
follow-up; the card reports the measured numbers with the storage-not-speed label.
