...
(`scripts/csd-publish-checkpoint.py:1042`) is the exact byte-for-byte shape every
...
`cogsyndelta.cards.render_card` (`src/cogsyndelta/cards/render.py:263`) is the newer,
...
`render_card` (`src/cogsyndelta/cards/render.py:263`) is the one public entry point.
