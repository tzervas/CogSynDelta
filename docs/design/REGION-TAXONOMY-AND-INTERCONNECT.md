...
**Status:** design draft, **REVISION 3.3**, for operator ratification. Nothing here is applied.
...
order**, which for once is also append order; the ordering was re-checked rather than assumed.
...
reservation required, AFTER DEC-49 ...............  67,584   of which 61,440 is TRAINING data
  ...
  67,584 as a share of what remains ..............   40.0%   <- the real figure (was 33.3% at
                                                                56,320; 67.6% before the recovery)
  text-usable vs the 61,440-row training need ....   1.77x SURPLUS  (was 2.13x at 51,200, and a
...
> this is visible arithmetic:** 67,584 of 169,047 clean unallocated is **40.0%** (was 33.3%), and
> the training need alone — 61,440 rows — leaves a surplus of **1.77×** against the 109,047
> text-usable rows, down from **2.13×**. **§9.2's funding risk is tightened, not closed**, and the
...
into a **1.77×** surplus at DEC-49's 61,440-row training need (2.13× at the old 51,200; 2.23×
before DEC-42's union burn), so *"this single generator can supply the entire training
...
**A property worth stating before the list: five of the six are answerable by BUILDING, and one is
not.** Gaps (a), (c), (d), (e) and (f) become measurements the moment E1 runs. Gap (b) — the
partition axis — is a policy question about who is isolated from whom, and no measurement decides
it. If only one of these is read closely, read (b).
...
`MemoryRecord`, `LearningContext`, or the store protocols [V]. **Persona/basin is specified and has
...
Started**, **no `Persona` or `Basin` class exists in either `src/` tree** [V, grep, zero hits
outside docs].
...
**zero hits in both `src/` trees** [V]. No offset, delta, diff, LoRA-style overlay, per-persona
weight delta or sparse update structure appears in either store [V]. The nearest analogues are
...
constructs a leak, and no capacity-exhaustion test anywhere in either repo [V].
...
**109,047 text rows** against a **61,440**-row training requirement after the aqua_rat recovery,
DEC-42's union burn and **DEC-49's reserve increase** (§5.1, §5.4) — a **1.77× surplus** where
revision 1 computed a 2.38× shortfall and revision 3.2 computed 2.13×. **DEC-49 spent a third of
...
required 67,584, of which 61,440 is training data     [DEC-49]   was 56,320 / 51,200
  as a share of what remains                            40.0%      was   33.3% (67.6% pre-recovery)
  vs the 61,440-row training need                        1.77x SURPLUS  (was 2.13x at 51,200,
                                                                        and 2.38x SHORTFALL before)
...
RESERVATION TOTAL                                         67,584      [I]   was 56,320
  as a share of the 169,047 clean unallocated               40.0%     [I]   was 33.3%
  surplus of 109,047 text-usable vs the 61,440 train need    1.77x    [I]   was 2.13x
