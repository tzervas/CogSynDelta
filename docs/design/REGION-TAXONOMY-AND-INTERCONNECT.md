...
| --- | --- | --- | --- |
| **Placement** | where a span sits | **yes**, and should — a dynamic capacity is the point | `_enforce_placement`, bounded by `capacity_bytes` |
| **Membership** | whether a record exists | **no** | `_enforce_membership`, bounded by `TierBudget.max_records` |

Four clauses, and the third and fourth are the ones most easily got wrong:
...
**Still open, and a separate decision:** read priority retains the GPU bonus and truncates at
`b_store`, so this amendment does **not** make eval reads identical across hosts. **Direction of
travel, recorded but not licensed here:** hot/warm capacity derived from the *model* — a fixed
record count as a ratio of model size, with cold unbounded and user-configured — which removes the
host dependence at its source rather than breaking one propagation path.
