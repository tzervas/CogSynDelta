...
class FisherInformationPredictor(nn.Module):
    ...
    def predict_weight_distribution(
        ...
        def data_gen() -> Iterator[tuple[Tensor, Tensor]]:
            """Generate data batches for Fisher matrix estimation.

            Yields:
                Tuple of training inputs and target tensors for each batch.
            """
    ...
    def compute_natural_gradient_update(
        ...
        if not self._fisher_cache:
            ...
            def data_gen() -> Iterator[tuple[Tensor, Tensor]]:
                """Generate data batches for natural gradient estimation.

                Yields:
                    Tuple of inputs and target tensors for each batch.
                """
                for i in range(0, len(train_x), 32):
                    yield train_x[i : i + 32], train_y[i : i + 32]
...
class UnifiedAlgebraicTrainer:
    ...
    def _optimize_mhc_modules(self, train_x: Tensor) -> dict[str, Any]:
        """Optimize auxiliary mHC modules algebraically.

        Args:
            train_x: Training inputs.

        Returns:
            Dictionary containing mHC analysis results.
        """
    ...
    def _optimize_pathway_strengths(self) -> dict[tuple[str, str], float] | None:
        """Optimize pathway strengths using interconnect information.

        Returns:
            Dictionary mapping pathway pairs to predicted strengths, or None.
        """
    ...
    def train_algebraically(
        ...
        if self.mhc_optimizer and self.mhc_modules:
            results["mhc_analysis"] = self._optimize_mhc_modules(train_x)

        # Pathway optimization
        if self.pathway_predictor and self.interconnect:
            pathway_strengths = self._optimize_pathway_strengths()
