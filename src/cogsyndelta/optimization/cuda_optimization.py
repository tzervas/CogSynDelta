...
class RTX5080Optimizer:
    ...
    def __init__(self) -> None:
        ...
        if not CUDA_AVAILABLE:
            ...
            self.config = GPUConfig(device=self.device, memory_gb=0, use_mixed_precision=False)
            return
...
class BalancedTernaryEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, embed_dim: int) -> None:
        ...
        # Store as int8 (-1, 0, 1)
        self.weight = nn.Parameter(torch.randint(-1, 2, (vocab_size, embed_dim), dtype=torch.int8))
...
class GPUEmbeddingStore:
    ...
    def __init__(
        ...
        optimizer: RTX5080Optimizer | None = None,
    ) -> None:
        ...
        if CUDA_AVAILABLE:
            if use_ternary:
                ...
                self.storage = torch.zeros(
                    max_capacity, embed_dim, dtype=torch.int8, device=self.optimizer.device
                )
            else:
                # FP16 storage
                dtype = (
                    torch.float16 if self.optimizer.config.use_mixed_precision else torch.float32
                )
                self.storage = torch.zeros(
                    max_capacity, embed_dim, dtype=dtype, device=self.optimizer.device
                )
        else:
            # CPU fallback
    ...
    def retrieve(self, embedding_id: str) -> torch.Tensor | None:
        """
